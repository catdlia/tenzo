...
struct BitLinearTL1LoweringToLinalg
    ...
    LogicalResult matchAndRewrite(tenzo::BitLinearTL1Op op, OpAdaptor adaptor,
        ...
        Value cn_blocks = rewriter.create<arith::ConstantIndexOp>(loc, N / 128); // 128 channels
        ...
        Value zeroF32Scalar = rewriter.create<arith::ConstantOp>(loc, rewriter.getFloatAttr(elemType, 0.0));
        ...
        auto nLoop = rewriter.create<scf::ParallelOp>(loc, ValueRange{c0}, ValueRange{cn_blocks}, ValueRange{c1}, ValueRange{},
            [&](OpBuilder &b, Location loc2, ValueRange ivs, ValueRange nArgs) {
                Value n_idx = b.create<arith::MulIOp>(loc2, ivs[0], b.create<arith::ConstantIndexOp>(loc2, 64)); // byte offset
                        ...
                        auto kInnerLoop = b2.create<scf::ForOp>(locOuter, k_outer_idx, k_outer_end, c1, curOuterAccs,
                            [&](OpBuilder &b3, Location loc3, Value k_half_idx, ValueRange curAccs) {
                                ...
                                for (int chunk = 0; chunk < 4; ++chunk) {
                                    Value byteOffset = b3.create<arith::AddIOp>(loc3, n_idx, b3.create<arith::ConstantIndexOp>(loc3, chunk * 32));
                ...
                for (int chunk = 0; chunk < 4; ++chunk) {
                    for (int sub = 0; sub < 4; ++sub) {
                        ...
                        Value n_actual = b.create<arith::AddIOp>(loc2, b.create<arith::MulIOp>(loc2, ivs[0], b.create<arith::ConstantIndexOp>(loc2, 128)), b.create<arith::ConstantIndexOp>(loc2, chunk * 32 + sub * 8));
...
content = content[:populate_idx] + new_pass + content[populate_idx:]

# Need to add the registration:
add_line = "    patterns.add<BitLinearTL1LoweringToLinalg>(patterns.getContext());"
reg_idx = content.find('patterns.add<BitLinearElutLoweringToLinalg>(patterns.getContext());')
if reg_idx != -1:
    end_of_line = content.find('\n', reg_idx)
    content = content[:end_of_line+1] + add_line + '\n' + content[end_of_line+1:]
else:
    print("Warning: could not find where to register")
