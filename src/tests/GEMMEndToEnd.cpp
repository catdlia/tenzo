...
void microKernel6x16(const float *__restrict__ A_packed,
                     const float *__restrict__ B_packed, float *__restrict__ C,
                     int ldc, int K) {
...
#undef KERNEL_ITER
  ...
  if (bias != nullptr) {
    ...
    c00 = _mm256_add_ps(c00, bias0);
    c01 = _mm256_add_ps(c01, bias1);
    c10 = _mm256_add_ps(c10, bias0);
    c11 = _mm256_add_ps(c11, bias1);
    c20 = _mm256_add_ps(c20, bias0);
    c21 = _mm256_add_ps(c21, bias1);
    c30 = _mm256_add_ps(c30, bias0);
    c31 = _mm256_add_ps(c31, bias1);
    c40 = _mm256_add_ps(c40, bias0);
    c41 = _mm256_add_ps(c41, bias1);
    c50 = _mm256_add_ps(c50, bias0);
    c51 = _mm256_add_ps(c51, bias1);
  }
...
//===----------------------------------------------------------------------===//
void gemmPerCoreIndependent(const float *A, const float *B, float *C, int M,
                            int N, int K, int lda, int ldb, int ldc,
                            const tenzo::TopologyInfo &topo, int aKC, int aMC,
                            int aNC,
                            tenzo::ThreadPool *pool = nullptr) {
  ...
  // Lambda for per-worker GEMM (shared by both paths)
  auto workerGemm = [&](const tenzo::HeterogeneousWorkSplit::ThreadWork &tw) {
        ...
        for (int icRel = 0; icRel < localM; icRel += aMC) {
          ...
          for (int jr = 0; jr < numMicroJ; jr++) {
            for (int ir = 0; ir < numMicroI; ir++) {
              ...
              if (rowsToWrite == MR && colsToWrite == NR) {
                microKernel6x16(aPtr, bPtr, C + cRow * ldc + cCol, ldc,
                                kcActual);
              } else {
                alignas(32) float tempC[MR * NR] = {0};
                for (int i = 0; i < rowsToWrite; i++)
  ...
  if (pool) {
    // Use persistent ThreadPool — workers already pinned to cores
    pool->executeSplit(split, workerGemm);
  } else {
    // Fallback: raw std::thread with manual affinity
    std::vector<std::thread> workers;
    workers.reserve(numWorkers);

    for (auto &tw : split.assignments) {
      workers.emplace_back([&, tw]() {
#ifdef __linux__
        if (tw.cpuId >= 0) {
          cpu_set_t cpuset;
          CPU_ZERO(&cpuset);
...
// Per-core-type blocking: each core type gets its own KC/MC/NC based on its
// cache
void gemmPerCoreIndependentPerType(
    const float *A, const float *B, float *C, int M, int N, int K, int lda,
    int ldb, int ldc,
    const std::shared_ptr<tenzo::HardwareProfile> &hwInfo) {

  auto split = tenzo::HeterogeneousWorkSplit::compute(M, hwInfo->getTopology());
  int numWorkers = (int)split.assignments.size();
  ...
  auto pParams =
      hwInfo->getOptimalMicroKernelParamsForCoreType(tenzo::CoreType::P_CORE);
  auto eParams =
      hwInfo->getOptimalMicroKernelParamsForCoreType(tenzo::CoreType::E_CORE);
...
//===----------------------------------------------------------------------===//
void gemmWithPacking_Parallel(const float *A, const float *B, float *C, int M,
                              int N, int K, int lda, int ldb, int ldc) {
      ...
      for (int ic = 0; ic < M; ic += MC) {
        ...
        for (int jr = 0; jr < numMicroJ; jr++) {
          for (int ir = 0; ir < numMicroI; ir++) {
            ...
            if (rowsToWrite == MR && colsToWrite == NR) {
              float *cPtr = C + cRow * ldc + cCol;
              microKernel6x16_BiasReLU(aPtr, bPtr, cPtr, ldc, kcActual, biasPtr,
                                       applyRelu);
            } else {
              alignas(32) float tempC[MR * NR] = {0};
...
//===----------------------------------------------------------------------===//
void gemmNaive(const float *A, const float *B, float *C, int M, int N, int K,
...
// Benchmark runner
//===----------------------------------------------------------------------===//
double runGemmBenchmark(void (*gemm)(const float *, const float *, float *, int,
                                     int, int, int, int, int),
  ...
  for (int i = 0; i < iterations; i++) {
    // Reset C before each iteration (outside timing for naive, but our kernel
    // accumulates) Actually for fair comparison, we should NOT reset - just
...
void runGEMMEndToEndBenchmark(mlir::MLIRContext &context) {
  ...
  llvm::outs() << "╔══════════════╦═══════════════════╦═══════════════════╦════"
                  "════════╗\n";
  llvm::outs() << "║   Size       ║  Naive (GFLOPS)   ║  Packed (GFLOPS)  ║   "
                  "Speedup  ║\n";
  llvm::outs() << "╠══════════════╬═══════════════════╬═══════════════════╬════"
                  "════════╣\n";
  ...
  for (const auto &test : tests) {
    ...
    double gflopsNaive =
        runGemmBenchmark(gemmNaive, test.M, test.N, test.K, 2, 5);

    // Run packed benchmark - more warmup and iterations for stable results
    double gflopsPacked =
        runGemmBenchmark(gemmWithPacking, test.M, test.N, test.K, 10, 50);
  ...
  llvm::outs() << "╔══════════════╦═══════════════════╦═══════════════════╦════"
                  "════════╗\n";
  llvm::outs() << "║   Size       ║  MatMul (GFLOPS)  ║  +Bias+ReLU       ║  "
                  "Overhead  ║\n";
  llvm::outs() << "╠══════════════╬═══════════════════╬═══════════════════╬════"
                  "════════╣\n";
  ...
  for (const auto &test : tests) {
    ...
    for (int i = 0; i < test.M * test.K; i++)
      A[i] = 0.5f;
    for (int i = 0; i < test.K * test.N; i++)
      B[i] = 0.5f;
    memset(C, 0, test.M * test.N * sizeof(float));

    // Warmup
    for (int i = 0; i < 10; i++) {
      gemmWithPacking_BiasReLU(A, B, C, test.M, test.N, test.K, test.K, test.N,
                               test.N, bias, true);
    }
  ...
  // ═══════════════════════════════════════════════════════════════════════
  llvm::outs()
      << "🚀 [Multi-threading] Single vs Parallel (Adaptive) Performance:\n";
  llvm::outs() << "   CPU: Intel Alder Lake (2 P-cores + 4 E-cores)\n";

  // Show actual thread config
  const char *envThreads = std::getenv("OMP_NUM_THREADS");
  int maxT = envThreads ? std::atoi(envThreads) : 4;
  llvm::outs() << "   Max threads: " << maxT
               << " (set via OMP_NUM_THREADS, default=4 for P-cores)\n";
  llvm::outs() << "   Strategy: Small=1T, Medium=2T, Large=" << maxT << "T\n\n";

  llvm::outs() << "╔══════════════╦═══════════════════╦═══════════════════╦════"
                  "════════╗\n";
  llvm::outs() << "║   Size       ║  Single (GFLOPS)  ║  Parallel (GFLOPS)║   "
                  "Speedup  ║\n";
  llvm::outs() << "╠══════════════╬═══════════════════╬═══════════════════╬════"
                  "════════╣\n";
  ...
  for (const auto &test : tests) {
    // Single-threaded
    double gflopsSingle =
        runGemmBenchmark(gemmWithPacking, test.M, test.N, test.K, 5, 30);

    // Parallel
    double gflopsParallel = runGemmBenchmark(gemmWithPacking_Parallel, test.M,
                                             test.N, test.K, 5, 30);
  ...
  // ═══════════════════════════════════════════════════════════════════════
  llvm::outs()
      << "⚡ [2-Core Parallel] Two Independent GEMMs (no sync overhead):\n";
  llvm::outs() << "   Each thread runs its own complete GEMM - perfect "
                  "parallelism test\n\n";
  ...
  llvm::outs() << "╔══════════════╦═══════════════════╦═══════════════════╦════"
                  "════════╗\n";
  llvm::outs() << "║   Size       ║  1-Core (GFLOPS)  ║  2-Core (GFLOPS)  ║   "
                  "Speedup  ║\n";
  llvm::outs() << "╠══════════════╬═══════════════════╬═══════════════════╬════"
                  "════════╣\n";
  ...
  for (int size : parallelSizes) {
    ...
    for (int i = 0; i < iters; i++) {
      std::thread thread1(
          [&]() { gemmWithPacking(A1, B1, C1, M, N, K, K, N, N); });
      std::thread thread2(
          [&]() { gemmWithPacking(A2, B2, C2, M, N, K, K, N, N); });
      thread1.join();
      thread2.join();
    }
  ...
  llvm::outs() << "   Philosophy: Each core runs its OWN complete GEMM — no "
                  "barriers, no shared data\n";

  if (hwInfo->getTopology().isHybrid()) {
    auto pParams =
        hwInfo->getOptimalMicroKernelParamsForCoreType(CoreType::P_CORE);
    auto eParams =
        hwInfo->getOptimalMicroKernelParamsForCoreType(CoreType::E_CORE);
    auto split = HeterogeneousWorkSplit::compute(1024, hwInfo->getTopology());
    llvm::outs() << "   Detected: " << hwInfo->getTopology().numPCores
                 << " P-cores (" << hwInfo->getTopology().threadsPerPCore
                 << " threads/core, "
                 << hwInfo->getTopology().pCoreMaxFreqKHz / 1000 << " MHz) + "
                 << hwInfo->getTopology().numECores << " E-cores ("
                 << hwInfo->getTopology().threadsPerECore << " thread/core, "
                 << hwInfo->getTopology().eCoreMaxFreqKHz / 1000 << " MHz)\n";
    llvm::outs() << "   Per-type blocking:\n";
    llvm::outs() << "     P-core: L1d="
                 << hwInfo->getTopology().pCoreCache.l1dKB
                 << "KB → KC=" << pParams.KC << " MC=" << pParams.MC
                 << " NC=" << pParams.NC << "\n";
    llvm::outs() << "     E-core: L1d="
                 << hwInfo->getTopology().eCoreCache.l1dKB
                 << "KB → KC=" << eParams.KC << " MC=" << eParams.MC
                 << " NC=" << eParams.NC << "\n";

    llvm::outs() << "   Work split (weighted by freq):\n";
    for (auto &tw : split.assignments) {
      const char *typeStr = tw.coreType == CoreType::P_CORE ? "P" : "E";
      llvm::outs() << "     Core " << tw.threadId << " [" << typeStr << ", cpu"
                   << tw.cpuId << "]: " << (tw.rowEnd - tw.rowStart)
                   << " rows\n";
    }
  } else {
  ...
  llvm::outs() << "\n╔══════════════╦═══════════════╦═══════════════╦══════════"
                  "═════╦═══════════════╗\n";
  llvm::outs() << "║   Size       ║  1T Hardcoded ║  N-Core HC    ║  N-Core "
                  "Adapt ║  N-Core PerTy ║\n";
  llvm::outs() << "╠══════════════╬═══════════════╬═══════════════╬════════════"
                  "═══╬═══════════════╣\n";
  ...
  for (const auto &test : tests) {
    ...
    for (int w = 0; w < 3; w++) {
      memset(C, 0, test.M * test.N * sizeof(float));
      gemmPerCoreIndependent(A, B, C, test.M, test.N, test.K, test.K, test.N,
                             test.N, hwInfo->getTopology(), KC, MC, NC);
    }
    ...
    double g2 = (2.0 * test.M * test.N * test.K /
                 (std::chrono::duration<double>(e - s).count() / 30)) /
                1e9;
    ...
    for (int w = 0; w < 3; w++) {
      memset(C, 0, test.M * test.N * sizeof(float));
      gemmPerCoreIndependent(A, B, C, test.M, test.N, test.K, test.K, test.N,
                             test.N, hwInfo->getTopology(), mkp.KC, mkp.MC,
                             mkp.NC);
    }
    ...
    for (int w = 0; w < 3; w++) {
      memset(C, 0, test.M * test.N * sizeof(float));
      gemmPerCoreIndependentPerType(A, B, C, test.M, test.N, test.K, test.K,
                                    test.N, test.N, hwInfo);
    }
    s = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < 30; i++)
      gemmPerCoreIndependentPerType(A, B, C, test.M, test.N, test.K, test.K,
  ...
  llvm::outs() << "╔══════════════╦═══════════════════╦═══════════════════╦════"
                  "═══════════════╦════════════╗\n";
  llvm::outs() << "║   Size       ║  BLAS 1T (GFLOPS) ║  BLAS MT (GFLOPS) ║  "
                  "Tenzo Best       ║   vs 1T    ║\n";
  llvm::outs() << "╠══════════════╬═══════════════════╬═══════════════════╬════"
                  "═══════════════╬════════════╣\n";
  ...
  for (const auto &test : tests) {
    ...
    for (int w = 0; w < 5; w++) {
      memset(C, 0, test.M * test.N * sizeof(float));
      cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, test.M, test.N,
                  test.K, 1.0f, A, test.K, B, test.N, 0.0f, C, test.N);
    }
    ...
    double gflops1T = (2.0 * test.M * test.N * test.K /
                       (std::chrono::duration<double>(e1 - s1).count() / 30)) /
                      1e9;

    // OpenBLAS multi-thread
    openblas_set_num_threads(hwInfo->getTopology().totalLogicalCpus());
    for (int w = 0; w < 5; w++) {
      memset(C, 0, test.M * test.N * sizeof(float));
      cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, test.M, test.N,
                  test.K, 1.0f, A, test.K, B, test.N, 0.0f, C, test.N);
    ...
    double gflopsMT = (2.0 * test.M * test.N * test.K /
                       (std::chrono::duration<double>(eM - sM).count() / 30)) /
                      1e9;
    ...
    double gflopsBest =
        (2.0 * test.M * test.N * test.K /
         (std::chrono::duration<double>(eT - sT).count() / 30)) /
        1e9;
  ...
  llvm::outs() << "╚══════════════╩═══════════════════╩═══════════════════╩════"
                  "═══════════════╩════════════╝\n\n";
#else
  llvm::outs() << "ℹ️  OpenBLAS not available (build with -DTENZO_HAS_OPENBLAS "
                  "and link -lopenblas)\n\n";
#endif

  llvm::outs() << "📊 Analysis:\n";
  llvm::outs() << "   - Per-core independent: each core runs full GEMM "
                  "pipeline, zero sync\n";
  llvm::outs() << "   - Adaptive blocking: KC=" << mkp.KC << " MC=" << mkp.MC
               << " NC=" << mkp.NC << " (from actual cache sizes)\n";
  llvm::outs() << "   - Hardcoded blocking: KC=" << KC << " MC=" << MC
               << " NC=" << NC << "\n";
  ...
  // ═══════════════════════════════════════════════════════════════════════
  llvm::outs()
      << "🔬 [Parameter Sweep] Finding optimal KC/MC/NC for 512×512:\n\n";
  ...
  for (int ci = 0; ci < 8; ci++) {
    ...
    for (int i = 0; i < sweepM * sweepK; i++)
      A[i] = 0.5f;
    for (int i = 0; i < sweepK * sweepN; i++)
      B[i] = 0.5f;

    // 1T benchmark
    for (int w = 0; w < 3; w++) {
      memset(C, 0, sweepM * sweepN * sizeof(float));
      gemmAdaptive(A, B, C, sweepM, sweepN, sweepK, sweepK, sweepN, sweepN,
    ...
    double g1t = (2.0 * sweepM * sweepN * sweepK /
                  (std::chrono::duration<double>(e - s).count() / 20)) /
                 1e9;

    // N-Core benchmark
    for (int w = 0; w < 2; w++) {
      memset(C, 0, sweepM * sweepN * sizeof(float));
      gemmPerCoreIndependent(A, B, C, sweepM, sweepN, sweepK, sweepK, sweepN,
                             sweepN, hwInfo->getTopology(), cfg.kc, cfg.mc,
                             cfg.nc);
    }
    s = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < 20; i++)
    ...
    char buf[256];
    snprintf(buf, sizeof(buf), "║  %s ║    %6.1f     ║    %6.1f     ║\n",
             cfg.label, g1t, gNc);
    llvm::outs() << buf;
    llvm::outs().flush();
