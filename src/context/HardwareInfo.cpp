...
static int parseCacheSizeKB(const std::string& s) {
    ...
    if (s.back() == 'M' || s.find('M') != std::string::npos) val *= 1024;
    return val;
}
...
static TopologyInfo detectTopology() {
    ...
    std::map<int, std::vector<int>> coreIdToCpus; // core_id -> list of cpus sharing it
    ...
    for (int cpu = 0; cpu < numCpus; cpu++) {
        ...
        auto siblings = parseCpuList(readSysfsFile(base + "/topology/thread_siblings_list"));
        ci.isHyperthread = (siblings.size() > 1 && cpu != siblings[0]);

        coreIdToCpus[ci.coreId].push_back(cpu);
        if (ci.maxFreqKHz > 0) {
            coreIdToMaxFreq[ci.coreId] = std::max(
                coreIdToMaxFreq[ci.coreId], ci.maxFreqKHz);
        }
    ...
    for (auto& [coreId, freq] : coreIdToMaxFreq) {
        ...
        if (freq > 0) globalMinFreq = std::min(globalMinFreq, freq);
    }
    ...
    for (auto& ci : topo.allCpus) {
        if (isHybrid) {
            ci.type = pCoreCoreIds.count(ci.coreId) ? CoreType::P_CORE : CoreType::E_CORE;
        } else {
    ...
    for (auto& ci : topo.allCpus) {
        if (ci.isHyperthread) continue; // Don't double-count HT
        if (ci.type == CoreType::P_CORE) uniquePCores.insert(ci.coreId);
        else if (ci.type == CoreType::E_CORE) uniqueECores.insert(ci.coreId);
    }
    ...
    // Count threads
    for (auto& ci : topo.allCpus) {
        if (ci.type == CoreType::P_CORE) topo.numPThreads++;
        else if (ci.type == CoreType::E_CORE) topo.numEThreads++;
    }
    ...
    auto readCacheForCpu = [](int cpuNum) -> CacheInfo {
        ...
        for (int idx = 0; idx < 4; idx++) {
            ...
            if (level == 1 && type == "Data")        c.l1dKB = sizeKB;
            else if (level == 1 && type == "Instruction") c.l1iKB = sizeKB;
            else if (level == 2)                     c.l2KB = sizeKB;
            else if (level == 3)                     c.l3KB = sizeKB;
    ...
    if (isHybrid) {
        ...
        if (pCoreCpu >= 0) topo.pCoreCache = readCacheForCpu(pCoreCpu);
        else               topo.pCoreCache = topo.cache;
        if (eCoreCpu >= 0) topo.eCoreCache = readCacheForCpu(eCoreCpu);
        else               topo.eCoreCache = topo.cache;
    } else {
...
// ============================================================
HardwareInfo::MicroKernelParams HardwareInfo::getOptimalMicroKernelParams() const {
    return getOptimalMicroKernelParamsForCoreType(CoreType::UNKNOWN);
}

HardwareInfo::MicroKernelParams HardwareInfo::getOptimalMicroKernelParamsForCoreType(
    CoreType type) const {
    MicroKernelParams p;

    // ---- MR, NR from ISA ----
    if (hasAVX512 && hasFMA) {
        p.MR = 14; p.NR = 32; p.unrollK = 8;
    } else if (hasAVX2 && hasFMA) {
        p.MR = 6; p.NR = 16; p.unrollK = 4;
    } else if (hasNEON) {
        p.MR = 8; p.NR = 12; p.unrollK = 4;
    } else {
        p.MR = 4; p.NR = 4; p.unrollK = 2;
    }
    ...
    if (numCores <= 0) numCores = physicalCores > 0 ? physicalCores : 1;
...
// ============================================================
HeterogeneousWorkSplit HeterogeneousWorkSplit::compute(
    int totalRows, const TopologyInfo& topo, bool usePCoresOnly) {

    HeterogeneousWorkSplit split;
    
    // We need MR for alignment to avoid partial tiles in the middle of the work split
    // Default to 6 if we can't detect it easily here, but ideally this should be a parameter
    int MR = 6; 
    ...
    // We'll use (pFreq * 2) for P-cores vs (eFreq * 1) for E-cores as a heuristic.
    float pWeight = (float)topo.pCoreMaxFreqKHz * 2.5f; 
    float eWeight = (float)topo.eCoreMaxFreqKHz * 1.0f;

    double totalCapacity = (double)pCpus.size() * pWeight +
                           (double)eCpus.size() * eWeight;
    ...
    for (int i = 0; i < (int)pCpus.size(); i++) {
        ...
        if (offset + rows > totalRows) rows = totalRows - offset;
        if (rows <= 0) break;
...
HardwareInfo HardwareInfo::detect() {
    ...
    info.hasAVX2 = features.lookup("avx2");
    info.hasAVX512 = features.lookup("avx512f");
    info.hasFMA = features.lookup("fma");
    info.hasVNNI = features.lookup("avxvnni") || features.lookup("avx512vnni");
    info.hasAMX = features.lookup("amx-tile");
    info.hasNEON = features.lookup("neon");

    info.logicalCores = std::thread::hardware_concurrency();

    // Detect full topology via sysfs
#ifdef __linux__
    info.topology = detectTopology();

    // Populate legacy fields from topology
    if (info.topology.isHybrid()) {
        info.physicalCores = info.topology.totalPhysicalCores();
    } else {
        // Homogeneous: count unique core_ids
        std::set<int> uniqueCores;
        for (auto& ci : info.topology.allCpus) {
            if (!ci.isHyperthread) uniqueCores.insert(ci.coreId);
        }
        info.physicalCores = uniqueCores.empty()
                             ? std::max(1, info.logicalCores / 2)
    ...
    if (maxFreq > 0) {
        info.turboFreqGHz = maxFreq / 1000000.0f;
    }
...
void HardwareInfo::print() const {
    ...
    if (topology.cache.l3KB > 0) {
        llvm::outs() << "   L3:  " << topology.cache.l3KB / 1024 << " MB (shared)\n";
    }
    ...
    if (topology.isHybrid()) {
        ...
        llvm::outs() << "   P-core: KC=" << pParams.KC
                     << " MC=" << pParams.MC << " NC=" << pParams.NC << "\n";
        llvm::outs() << "   E-core: KC=" << eParams.KC
                     << " MC=" << eParams.MC << " NC=" << eParams.NC << "\n";
    } else {
    ...
    if (topology.isHybrid()) {
        ...
        for (auto& tw : split.assignments) {
            ...
            llvm::outs() << "   Thread " << tw.threadId
                         << " [" << typeStr << "-core, cpu" << tw.cpuId << "]: rows "
                         << tw.rowStart << ".." << tw.rowEnd
