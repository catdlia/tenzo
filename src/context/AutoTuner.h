...
public:
  ...
  MicroKernelParams tune(const std::shared_ptr<tenzo::HardwareProfile> &hwInfo);
...
private:
  ...
  /// Generates a list of candidate parameters based on hardware limits
  /// (registers)
  std::vector<MicroKernelParams>
  generateCandidates(const std::shared_ptr<tenzo::HardwareProfile> &hwInfo);

  /// Benchmarks a specific configuration
  TuningResult benchmark(const MicroKernelParams &params,
                         const std::shared_ptr<tenzo::HardwareProfile> &hwInfo);

  /// Generates a small MLIR module for the micro-kernel to test
  mlir::OwningOpRef<mlir::ModuleOp>
  generateTestModule(const MicroKernelParams &params);
};
