...
docker compose -f "$PROJECT_ROOT/docker-compose.yml" run --rm -e OMP_PLACES=cores -e OMP_PROC_BIND=spread dev bash -c "\
    ...
    echo '---BUILD COMPLETE---' && \
    ls -lh /app/cmake-build-debug/tenzo-cli" > "$PROJECT_ROOT/packing_build.log" 2>&1

if grep -q "BUILD COMPLETE" "$PROJECT_ROOT/packing_build.log"; then
    ...
    tail -50 "$PROJECT_ROOT/packing_build.log"
fi
